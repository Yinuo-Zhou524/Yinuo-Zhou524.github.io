---
layout: default
title: Home
---

<div class="hero">
  <div class="hero-text">
    <h1>Yinuo Zhou</h1>
    <p class="subtitle">Ph.D. Candidate in Renewable Natural Resources, Louisiana State University</p>

    <p>
      I study tree physiology, forest health, and climate-driven stress responses, with a focus on
      bark water vapor conductance, flooding, drought, and mechanisms of tree mortality.
    </p>

    <p>
      My work combines field studies, greenhouse experiments, and laboratory measurements to understand
      how woody tissues respond to environmental stress.
    </p>

    <div class="button-row">
      <a class="button" href="/research.html">Research</a>
      <a class="button" href="/publications.html">Publications</a>
      <a class="button" href="/cv.html">CV</a>
      <a class="button" href="/contact.html">Contact</a>
    </div>
  </div>

  <div class="hero-photo">
    <img src="/assets/img/profile.JPG" alt="Portrait of Yinuo Zhou">
  </div>
</div>

<section>
  <h2>About</h2>
  <p>
    <h3 class="Chinese">你好！</h3>
    
    Welcome to my website :smile_cat: I am interested in how trees regulate non-foliar water loss and how
    bark traits influence responses to heat, flooding, and drought. My broader goal is to better understand
    tree resilience under changing environmental conditions.
  </p>
</section>

<section>
  <h2>Current Research Themes</h2>
  <ul>
    <li>Bark water vapor conductance and its anatomical controls</li>
    <li>Temperature sensitivity of residual water loss pathways</li>
    <li>Flooding-induced lenticel hypertrophy and bark function</li>
    <li>Tree stress physiology and forest health under climate extremes</li>
  </ul>
</section>

<section>
  <h2>News</h2>
  <ul>
    <li><strong>2026:</strong> Add your newest conference, paper, or project update here.</li>
    <li><strong>2025:</strong> </li>
    <li>12/15 Received [the Clark M. Hoffpauer Memorial Fund Scholarship](https://www.lsu.edu/agriculture/rnr/programs/scholarships.php)</li> 
    <li>11/04 Passed the General Exam. PHD CANDIDATE NOW!</li>
    <li>Add another recent milestone here.</li>
  </ul>
</section>